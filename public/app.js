...
let playbackToken=0, creatingPlaylist=false, liveSource=null;
function safeJSON(key,fallback){try{const v=JSON.parse(localStorage.getItem(key));return v??fallback}catch{return fallback}}
const CATALOG_CACHE_KEY='synthCatalog:v24';
const PLAYLIST_CACHE_KEY=u=>'synthPlaylists:v24:'+String(u||'').toLowerCase();
function cacheCatalog(){try{localStorage.setItem(CATALOG_CACHE_KEY,JSON.stringify(songs));localStorage.setItem(PLAYLIST_CACHE_KEY(user),JSON.stringify(playlists))}catch{}}
function loadCachedCatalog(){const cs=safeJSON(CATALOG_CACHE_KEY,[]),ps=safeJSON(PLAYLIST_CACHE_KEY(user),[]);if(Array.isArray(cs))songs=cs;if(Array.isArray(ps))playlists=ps;return songs.length||playlists.length;}
...
const resourceUrl=u=>{
  const raw=String(u||'');if(!raw)return '';
  try{
    if(raw.startsWith('/'))return (IS_NATIVE_APP?API_BASE:window.location.origin)+raw;
    const x=new URL(raw,window.location.href),host=x.hostname;
    if(!IS_NATIVE_APP&&(host==='localhost'||host==='127.0.0.1'||/^10\./.test(host)||/^192\.168\./.test(host)||/^172\.(1[6-9]|2\d|3[0-1])\./.test(host)))return window.location.origin+x.pathname+x.search;
    return x.href;
  }catch{return raw.startsWith('/')?(IS_NATIVE_APP?API_BASE:window.location.origin)+raw:raw}
};
...
async function migrateLegacyDownloads(){for(const s of songs){const raw=localStorage.getItem('download:'+s.id);if(!raw||!raw.startsWith('data:'))continue;try{const r=await fetch(raw);const b=await r.blob();await putOffline(s.id,b,s);localStorage.removeItem('download:'+s.id)}catch{}}}
async function init(){try{const c=await api('/api/config');ownerEmail=String(c.ownerEmail||'').trim().toLowerCase()}catch{}ownerCandidate=!!user&&!!ownerEmail&&user.trim().toLowerCase()===ownerEmail;if(ownerKey){try{await api('/api/admin/stats',{headers:{'x-admin-key':ownerKey}});owner=true}catch{owner=false;ownerKey='';localStorage.removeItem('synthOwnerKey');localStorage.removeItem('synthOwnerEmail')}}if(user){ownerCandidate=!!ownerEmail&&user.trim().toLowerCase()===ownerEmail;favorites=new Set(safeJSON('favorites:'+user,[]));recent=safeJSON('recent:'+user,[]);listened=Number(localStorage.getItem('listenedSeconds:'+user)||0);loadCachedCatalog();await loadOfflineCatalog();render();try{await refreshData()}catch{toast(navigator.onLine?'Could not connect to SYNTH AUDIO server':'Offline mode — downloaded music is available')}}else renderLogin();if(location.hash==='#owner')ownerGate()}
...
async function refreshData({silent=false}={}){
  ...
  if(!normalizedSongs.length && !navigator.onLine && songs.length) { /* keep cached catalog */ } else songs=normalizedSongs;
  if(!normalizedPlaylists.length && !navigator.onLine && playlists.length) { /* keep cached playlists */ } else playlists=normalizedPlaylists;
  cacheCatalog();
...
else if(page==='downloads'){const renderTarget=m;renderTarget.innerHTML=`<div class="pageIntro"><h1>Downloads</h1><p class="sub">Available offline on this device. ${navigator.onLine?'':'You are offline — downloaded songs remain playable.'}</p></div><div class="list"><p class="sub emptyText">Loading downloads…</p></div>`;Promise.all([getOfflineCatalog(),Promise.all(songs.map(async s=>[s,await hasOffline(s.id)]))]).then(([metas,items])=>{if(page!=='downloads'||renderTarget!==$('#pageContent'))return;const byId=new Map(songs.map(s=>[s.id,s]));const ds=[];for(const m of metas){const s=byId.get(m.id)||m;if(s&&!ds.some(x=>x.id===s.id))ds.push(s)}for(const [s,ok] of items)if(ok&&!ds.some(x=>x.id===s.id))ds.push(s);renderTarget.querySelector('.list').innerHTML=ds.map(row).join('')||'<p class="sub emptyText">No downloaded songs yet.</p>'}).catch(()=>{if(renderTarget=== $('#pageContent'))renderTarget.querySelector('.list').innerHTML='<p class="sub emptyText">Could not read offline downloads.</p>'})}
else if(page==='activity')renderActivity(m);
...
function renderActivity(m){api('/api/user/stats?userEmail='+encodeURIComponent(user)).then(st=>{listened=Number(st.listeningSeconds||0);localStorage.setItem('listenedSeconds:'+user,String(listened));const events=st.events||[];m.innerHTML=`<div class="pageIntro"><h1>Activity</h1><p class="sub">Your recent listening and library activity.</p></div><section><div class="sectionhead"><h2>Recent activity</h2></div><div class="activitylist">${events.slice(0,50).map(activityItem).join('')||'<p class="sub emptyText">No activity yet.</p>'}</div></section>`}).catch(()=>{const events=[];m.innerHTML=`<div class="pageIntro"><h1>Activity</h1><p class="sub">Your recent listening and library activity.</p></div><section><div class="sectionhead"><h2>Recent activity</h2></div><div class="activitylist"><p class="sub emptyText">You are offline. Activity will sync when you reconnect.</p></div></section>`})}
...
function go(p){if(p==='stats'||p==='activityStats')p='activity';if(p==='admin'&&!(owner&&ownerCandidate)){if(ownerCandidate){ownerGate();return}return go('home')}page=p;const hash=p.startsWith('playlist:')?'#playlist-'+p.slice(9):'#'+p;history.pushState({},'',location.pathname+hash);render()}
window.addEventListener('popstate',()=>{if(location.hash.startsWith('#playlist-')){page='playlist:'+location.hash.slice(10);render()}else if(location.hash==='#playlists'){page='playlists';render()}else if(location.hash==='#activityStats'||location.hash==='#activity'||location.hash==='#stats'){page='activity';render()}else if(location.hash==='#admin'){page='admin';render()}else{page='home';render()}});
...
function offlineDB(){return new Promise((resolve,reject)=>{const r=indexedDB.open(IDB_NAME,2);r.onupgradeneeded=()=>{const db=r.result;if(!db.objectStoreNames.contains(IDB_STORE))db.createObjectStore(IDB_STORE);if(!db.objectStoreNames.contains(IDB_META))db.createObjectStore(IDB_META)};r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)})}
...
async function getOfflineCatalog(){try{const db=await offlineDB();return await new Promise((resolve,reject)=>{const q=db.transaction(IDB_META,'readonly').objectStore(IDB_META).getAll();q.onsuccess=()=>resolve(Array.isArray(q.result)?q.result:[]);q.onerror=()=>reject(q.error)})}catch{return []}}
async function putOffline(id,blob,meta=null){const db=await offlineDB();return new Promise((resolve,reject)=>{const tx=db.transaction([IDB_STORE,IDB_META],'readwrite');tx.objectStore(IDB_STORE).put(blob,id);if(meta)tx.objectStore(IDB_META).put({...meta,id},id);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error||Error('Offline storage failed'))})}
...
async function loadOfflineCatalog(){
  const metas=await getOfflineCatalog();
  if(!metas.length)return false;
  const existing=new Map(songs.map(s=>[s.id,s]));
  for(const m of metas){offlineLabelCache.add(m.id);if(!existing.has(m.id))songs.push(m);else Object.assign(existing.get(m.id),m)}
  cacheCatalog();
  return true;
}
...
async function startSong(s){
  ...
  stopNativePolling();
  const blob=await getOffline(s.id);
  if(token!==playbackToken||current?.id!==s.id)return;
  if(blob){
    await destroyNativePlayer();
    if(token!==playbackToken)return;
    if(!offlineUrls.has(s.id))offlineUrls.set(s.id,URL.createObjectURL(blob));
    audio.pause();audio.src=offlineUrls.get(s.id);audio.currentTime=0;audio.load();
    touchRecent(s.id);sendEvent('play',s.id);renderPlayer();
    try{await audio.play()}catch{renderPlayer();toast('Tap Play to start playback')}
    return;
  }
  if(nativeMode()){
    try{audio.pause();audio.removeAttribute('src');audio.load()}catch{}
    touchRecent(s.id);sendEvent('play',s.id);renderPlayer();
    if(await startNativeSong(s)){if(token!==playbackToken)return;startListenTimer();renderPlayer();return}
    toast('Background audio could not start. Trying normal playback…');
  }
  await destroyNativePlayer();
  if(token!==playbackToken)return;
  audio.src=resourceUrl(s.file);audio.currentTime=0;audio.load();touchRecent(s.id);sendEvent('play',s.id);renderPlayer();
  try{await audio.play()}catch{renderPlayer();toast('Tap Play to start playback')}
}
...
function more(id){
  ...
  closeModal();
  const d=document.createElement('div');d.className='modal actionModal';
  d.innerHTML=`<div class="modalbox actionBox" role="dialog" aria-modal="true"><div class="modalTitle"><h3>Song actions</h3><button class="modalClose" aria-label="Close">${svg('close')}</button></div><button data-act="play">${svg('play')} Play</button><button data-act="next">${svg('next')} Play next</button><button data-act="queue">${svg('queue')} Add to queue</button><button data-act="playlist">${svg('playlist')} Add to playlist</button><button data-act="fav">${svg('heart')} ${favorites.has(id)?'Remove from favorites':'Add to favorites'}</button><button data-act="download">${svg('download')} ${awaitableOfflineLabel(id)}</button></div>`;
  const run=act=>{d.remove();if(act==='play')playSong(id);else if(act==='next')nextAdd(id);else if(act==='queue')addToQueue(id);else if(act==='playlist')addToPlaylist(id);else if(act==='fav')toggleFav(id);else if(act==='download')downloadSong(id)};
  d.querySelector('.modalClose').onclick=()=>d.remove();d.addEventListener('click',e=>{if(e.target===d)d.remove()});
  d.querySelectorAll('[data-act]').forEach(b=>b.onclick=()=>run(b.dataset.act));
  document.body.appendChild(d);
}
function awaitableOfflineLabel(id){return offlineLabelCache.has(id)?'Downloaded':'Download'}
const offlineLabelCache=new Set();
async function refreshOfflineLabel(id){if(await hasOffline(id))offlineLabelCache.add(id);else offlineLabelCache.delete(id)}
...
async function createPlaylistFromForm(){
  ...
  const name=$('#newPlaylistName')?.value.trim();if(!name)return toast('Enter a playlist name');
  const modal=document.querySelector('.modal'), btn=modal?.querySelector('.primary');
  creatingPlaylist=true;if(btn){btn.disabled=true;btn.classList.add('loading');btn.textContent='Creating…'}
  let image='';const f=$('#newPlaylistImage')?.files?.[0];if(f)image=await fileData(f);
  const clientRequestId=(crypto.randomUUID?.()||String(Date.now())+'-'+Math.random());
  try{const p=await api('/api/playlists',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,image,userEmail:user,clientRequestId})});playlists=await api('/api/playlists?userEmail='+encodeURIComponent(user));cacheCatalog();modal?.remove();go('playlist:'+p.id);toast('Playlist created')}catch(e){toast(e.message)}finally{creatingPlaylist=false}
}
...
async function downloadSong(id){
  const s=songs.find(x=>x.id===id);if(!s)return;
  if(await hasOffline(id))return toast('Already downloaded');
  toast('Downloading…');
  try{
    const b=await fetchBlobWithProgress(resourceUrl(s.file));
    await putOffline(id,b,s);offlineLabelCache.add(id);cacheCatalog();sendEvent('download',id);toast('Downloaded for offline listening');if(page==='downloads')render();
  }catch(e){const msg=String(e?.name||'').includes('Quota')?'Download failed: device browser storage is full. Remove an old download and try again.':'Download failed. Check your connection and try again.';toast(msg)}
}
function fetchBlobWithProgress(url){return new Promise((resolve,reject)=>{const x=new XMLHttpRequest();x.open('GET',url,true);x.responseType='blob';x.onprogress=e=>{if(e.lengthComputable&&e.total>0)document.title=`SYNTH AUDIO · ${Math.round(e.loaded/e.total*100)}%`};x.onload=()=>{document.title='SYNTH AUDIO';if(x.status>=200&&x.status<300)resolve(x.response);else reject(Error('Server returned '+x.status))};x.onerror=()=>{document.title='SYNTH AUDIO';reject(Error('Network error'))};x.onabort=()=>{document.title='SYNTH AUDIO';reject(Error('Download cancelled'))};x.send()})}
...
async function upload(){
  ...
  const msg=$('#uploadMsg');if(msg)msg.textContent=`Uploading ${formatBytes(f.size)}… 0%`;
  const q=new URLSearchParams({filename:f.name,title:$('#utitle')?.value||'',artist:$('#uartist')?.value||'',album:$('#ualbum')?.value||'',language:$('#ulang')?.value||'',genre:$('#ugenre')?.value||''});
  try{
    const added=await xhrUpload(API_BASE+'/api/admin/upload-stream?'+q.toString(),f,ownerKey,p=>{if(msg)msg.textContent=`Uploading ${formatBytes(f.size)}… ${p}%`});
    songs=[added,...songs.filter(s=>s.id!==added.id)];cacheCatalog();
    const coverFile=$('#ucov')?.files?.[0];
    if(coverFile){
      const cover=await fileData(coverFile);
      await api('/api/admin/song-cover/'+added.id,{method:'POST',headers:{'x-admin-key':ownerKey,'Content-Type':'application/json'},body:JSON.stringify({cover})});
      added.cover=added.cover||'';
    }
    if(msg)msg.textContent=added.cover?'Upload complete.':'Upload complete. Artwork is processing in the background if needed.';
...
function xhrUpload(url,file,key,onProgress){return new Promise((resolve,reject)=>{const x=new XMLHttpRequest();x.open('POST',url,true);x.setRequestHeader('x-admin-key',key);x.upload.onprogress=e=>{if(e.lengthComputable)onProgress(Math.min(100,Math.round(e.loaded/e.total*100)))};x.onload=()=>{if(x.status>=200&&x.status<300){try{resolve(JSON.parse(x.responseText))}catch{reject(Error('Invalid server response'))}}else{try{reject(Error(JSON.parse(x.responseText).error||'Upload failed'))}catch{reject(Error('Upload failed'))}}};x.onerror=()=>reject(Error('Network error'));x.onabort=()=>reject(Error('Upload cancelled'));x.send(file)})}
...
async function adminActivity(){const b=$('#adminbody');try{const st=await api('/api/admin/stats',{headers:{'x-admin-key':ownerKey}});const events=st.events.map(activityItem).join('');b.innerHTML=`<div class="adminPanel"><div class="adminPanelHead"><div><h2>Activity</h2><p class="sub">Recent listening and library activity.</p></div><span class="countPill">${st.events.length} events</span></div><div class="activitylist adminActivityList">${events||'<div class="adminEmpty compact"><h3>No activity yet</h3><p>Listening and library events will appear here.</p></div>'}</div></div>`}catch{toast('Could not load admin activity')}}
...
function startLiveRefresh(){
  ...
  const refresh=()=>{if(!user||document.hidden||!navigator.onLine||refreshing)return;refreshing=true;refreshData({silent:true}).then(({changed})=>{if(changed){renderPage();renderPlayer()}}).catch(()=>{}).finally(()=>{refreshing=false})};
  window.addEventListener('focus',refresh,{passive:true});document.addEventListener('visibilitychange',()=>{if(!document.hidden)refresh()});window.addEventListener('online',refresh,{passive:true});
  timer=setInterval(refresh,5000);
  const connect=()=>{if(!user||!navigator.onLine||liveSource)return;try{liveSource=new EventSource((API_BASE||location.origin)+'/api/events/stream');liveSource.onmessage=refresh;liveSource.addEventListener('catalog',refresh);liveSource.addEventListener('song_updated',e=>{try{const s=JSON.parse(e.data).song;if(s){songs=[s,...songs.filter(x=>x.id!==s.id)];cacheCatalog();renderPage();renderPlayer()}}catch{}});liveSource.onerror=()=>{liveSource?.close();liveSource=null;setTimeout(connect,5000)}}catch{liveSource=null}};
  window.addEventListener('online',connect,{passive:true});connect();
}
