...
function parseDataUrlCover(value){const m=/^data:([^;,]+)?;base64,(.+)$/s.exec(String(value||''));if(!m)return null;try{return {mime:m[1]||'image/jpeg',data:Buffer.from(m[2],'base64')}}catch{return null}}
...
const server=http.createServer((req,res)=>handle(req,res).catch(e=>send(res,500,{error:e.message||'Server error'})));
